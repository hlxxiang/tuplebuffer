    ...
    namespace Configuration
    {
        ...
        std::shared_ptr<Monster::Tuple> MonsterEncode(const std::shared_ptr<Monster>& obj)
        {
            if (obj != nullptr)
            {
                ...
                return make_shared<Monster::Tuple>(obj->monsterId, obj->name, obj->level, skillsArr, obj->model, Vector3Encode(obj->pos), obj->head);
            }
            else
            {
                return nullptr;
        ...
        std::shared_ptr<Monster> MonsterDecode(const std::shared_ptr<Monster::Tuple>& t)
        {
            ...
            if (t != nullptr)
            {
                ...
                obj->model = std::get<4>(*t.get());
                obj->pos = Vector3Decode(std::get<5>(*t.get()));
                obj->head = std::get<6>(*t.get());
            }
            return obj;
        }
