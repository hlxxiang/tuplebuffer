...
class AccessCPP extends cpp_1.CPP {
    ...
    compileGroupTypes(prefix, group, channelDefine) {
        ...
        for (const pair of channelDefine) {
            ...
            content += `\n\n${compile_1.T}namespace ${prefix}${pair[1]}${"Names"}\n${compile_1.T}{\n`;
            if (channels != null) {
                for (const record of channels) {
                    ...
                    content += `${compile_1.T}${compile_1.T}const string ${record.name} = \"${record.name}\";\n`;
                }
            }
            content += `${compile_1.T}}`;
            content += `\n\n${compile_1.T}namespace ${prefix}${pair[1]}\n${compile_1.T}{\n`;
            if (channels != null) {
                for (const record of channels) {
                    ...
                    content += `${compile_1.T}${compile_1.T}using ${record.name} = ${this.className(meta, true)};\n`;
                }
            }
            content += `${compile_1.T}};`;
        }
