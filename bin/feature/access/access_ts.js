...
class AccessTS extends ts_1.TS {
    ...
    compileGroupTypes(prefix, group, channelDefine) {
        ...
        for (const pair of channelDefine) {
            ...
            content += `${compile_1.T}const enum ${prefix}${pair[1]}${"Names"} {\n`;
            if (channels != null) {
                for (const record of channels) {
                    content += `${compile_1.T}${compile_1.T}${record.name} = \"${record.name}\",\n`;
                }
            }
            content += `${compile_1.T}}\n\n`;
            content += `${compile_1.T}interface ${prefix}${pair[1]} {\n`;
            if (channels != null) {
                for (const record of channels) {
                    let meta = record.meta;
                    content += `${compile_1.T}${compile_1.T}\"${record.name}\": ${this.className(meta)},\n`;
                }
            }
            content += `${compile_1.T}}\n\n`;
        }
